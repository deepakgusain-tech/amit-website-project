...
import { testimonialSchema, userSchema } from "@/lib/validators";
...
const TestimonialForm = ({ data, update = false }: TestimonialFormProps) => {
  const router = useRouter();
  const id = data?.id;

  const form = useForm<z.infer<typeof testimonialSchema>>({
    resolver: zodResolver(testimonialSchema),
    defaultValues: data || testimonialDefaultValues,
  });

  const [isPending, startTransition] = React.useTransition();
  const [preview, setPreview] = useState<string | null>(null);

  const onSubmit: SubmitHandler<z.infer<typeof testimonialSchema>> = async (
    values: any,
  ) => {
    startTransition(async () => {
      ...
      if (update && id) {
        res = await updateTestimonial(values, id);
      } else {
        res = await createTestimonial(values);
      }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input id="designation" placeholder="Enter designation" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input id="company" placeholder="Enter company" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input id="tag" placeholder="Enter tag" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
